...
async function UpdatePasswordFunction(req, res) {
  ...
  fetch_user_public_model(newEmail).then((fetchEmailResponse) => {
    ...
    login_model(payload).then((LoginResponse) => {
      ...
      let payload = { newEmail, newPassword };

      UpdatePasswordModel(payload).then((UpdatePasswordResponse) => {
        ...
        password_update_public_model(payload).then((PasswordPublicResponse) => {
          if (PasswordPublicResponse.error) {
            return res.send(
              responseObject(PasswordPublicResponse.error.message, false, null)
            );
          }
